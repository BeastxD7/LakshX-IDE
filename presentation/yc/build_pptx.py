...
kicker(s, "Product pitch — Y Combinator", top=Inches(1.0))
...
# ============================================================ SLIDE 15 — WHY YC
s = new_slide()
kicker(s, "Why YC, why now")
headline(s, "This deck describes a shipped prototype, not a slide deck of promises — and a swing big enough to be worth chasing.",
         size=26, height=Inches(1.7))
why_yc = [
    ("A working prototype, not a pitch",
     "Royal Mode's gated INTAKE→PLAN→EXECUTE→VERIFY phase machine, background subagents dispatching and monitoring parallel work, and an always-on local trace store are all shipped and running today — built by a small, fast-moving team, not described in a roadmap slide."),
    ("The big swing YC looks for",
     "Enterprise Agent Brain — one shared context across an entire engineering team's agents, not one developer's laptop — is the ambitious version of this company. The shipped single-player product is the evidence this team can actually ship toward it, not just describe it."),
]
for i, (label, desc) in enumerate(why_yc):
    card(s, Inches(0.8 + i * 6.0), Inches(3.0), Inches(5.7), Inches(2.9), label, desc, label_size=14, desc_size=12)
add_text(s, Inches(0.8), Inches(6.15), Inches(11.5), Inches(0.9),
         "We're not asking YC to fund an idea — we're asking YC to help a team that already ships turn a real "
         "foundation into the ambitious version of itself.",
         size=12.5, font=SANS, color=MUTED, line_spacing=1.3)

# ============================================================ SLIDE 16 — THE ASK
s = new_slide()
kicker(s, "What we're applying for — Y Combinator")
headline(s, "GTM speed for the product that's already shipping, and the network to make the enterprise bet real.",
         size=24, height=Inches(1.5))
asks = [
    ("Speed and rigor on GTM",
     "Turning five design-partner teams into a repeatable enterprise motion — the batch's cadence and pressure-testing is exactly the forcing function for going from “a few teams like it” to a real, sellable motion."),
    ("Network for the Agent Brain motion",
     "Warm intros into engineering orgs that already feel this coordination problem, other YC founders solving adjacent multi-agent/coordination problems worth comparing notes with, and the fundraising signal that follows a YC batch — all specific to derisking the enterprise bet."),
]
for i, (label, desc) in enumerate(asks):
    card(s, Inches(0.8 + i * 6.0), Inches(2.9), Inches(5.7), Inches(2.9), label, desc, label_size=13.5, desc_size=12)
add_text(s, Inches(0.8), Inches(6.1), Inches(11), Inches(0.8),
