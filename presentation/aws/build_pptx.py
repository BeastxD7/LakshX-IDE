...
kicker(s, "Product pitch — AWS Activate", top=Inches(1.0))
...
# ============================================================ SLIDE 15 — WHY AWS
s = new_slide()
kicker(s, "Why this is an AWS-shaped ask")
headline(s, "Agent Brain isn't a feature we bolt on. It's a compute and storage problem — the kind AWS Activate exists to fund.",
         size=23, height=Inches(1.6))
card(s, Inches(0.8), Inches(2.9), Inches(5.7), Inches(3.0), "A shared trace/index service is infrastructure, full stop",
     "A project-level trace and index store that every teammate's agent queries means a real multi-tenant backend — "
     "compute for indexing and query, storage for a growing trace history, networking and auth in front of it. "
     "That doesn't run on a laptop or ship as a client-side feature; it needs real cloud infrastructure to exist at all.",
     label_size=13, desc_size=11.5)
card(s, Inches(6.75), Inches(2.9), Inches(5.7), Inches(3.0), "We're extending something that already runs — not speculating from zero",
     "The local trace store and background-subagent registry are shipped and running today, on a single machine. "
     "A cloud-hosted, multi-tenant Agent Brain is the direct next step of infrastructure already proven to work, "
     "not an unproven bet.",
     label_size=13, desc_size=11.5)
add_text(s, Inches(0.8), Inches(6.15), Inches(11.5), Inches(0.8),
         "Compute credits are the difference between “the trace/index service exists in our architecture doc” and "
         "“the trace/index service exists” — for real, at the scale a pilot cohort's traffic actually generates.",
         size=12, font=SANS, color=MUTED, line_spacing=1.25)

# ============================================================ SLIDE 16 — THE ASK
s = new_slide()
kicker(s, "What we're applying for — AWS Activate")
headline(s, "Compute for the infrastructure the multiplayer bet needs, and the architecture guidance to build it safely.",
         size=23, height=Inches(1.6))
card(s, Inches(0.8), Inches(2.9), Inches(5.7), Inches(2.7), "Compute credits",
     "Compute credits for the trace/index infrastructure a team-scale Agent Brain runs on, at the scale a real pilot "
     "cohort needs — indexing, query, and storage for a shared, always-growing trace history across every teammate's "
     "agent, not a single laptop's workload.",
     label_size=13, desc_size=11.5)
card(s, Inches(6.75), Inches(2.9), Inches(5.7), Inches(2.7), "Architecture & solutions-architect support",
     "AWS Activate's technical guidance — solutions-architect access — to design that multi-tenant service properly "
     "the first time. A shared, cross-teammate trace index literally contains everyone's tool-call history; getting "
     "the tenancy and isolation model right at the design stage matters more than fixing it after an incident.",
     label_size=13, desc_size=11.5)
add_text(s, Inches(0.8), Inches(5.85), Inches(11), Inches(0.8),
