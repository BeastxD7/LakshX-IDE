...
kicker(s, "Product pitch — Microsoft for Startups Founders Hub", top=Inches(1.0))
...
# ============================================================ SLIDE 15 — WHY MICROSOFT
s = new_slide()
kicker(s, "Built on your platform, on purpose")
headline(s, "LakshX is a VS Code fork. That's not incidental — it's the whole distribution thesis.",
         size=26, height=Inches(1.5))
why_msft = [
    ("Every VS Code user is a zero-friction install", "Familiar keybindings, familiar extension model, familiar everything — the entire VS Code user base is the addressable market on day one, not a migration we have to sell."),
    ("Open VSX is a security choice, not a snub", "Extensions install from Open VSX rather than the Microsoft Marketplace — a deliberate supply-chain call (an id squatted on the wrong registry is a real attack, not a hypothetical one), fully documented in our own architecture docs. Happy to walk through it — and open to whatever the right long-term answer looks like as a Microsoft-aligned product."),
]
for i, (label, desc) in enumerate(why_msft):
...
add_text(s, Inches(0.8), Inches(5.9), Inches(11.5), Inches(1.0),
         "The bet: the best distribution channel for an agentic IDE is the IDE 70%+ of developers already have "
         "installed — and the best way to earn a place in that ecosystem is being genuinely useful inside it, not routing around it.",
         size=12.5, font=SANS, color=MUTED, line_spacing=1.3)

# ============================================================ SLIDE 16 — THE ASK
s = new_slide()
kicker(s, "What we're applying for — Microsoft for Startups Founders Hub")
headline(s, "Azure credits to host the multiplayer bet; the ecosystem to distribute the single-player product already shipping.",
         size=22, height=Inches(1.6))
asks = [
    ("Azure credits", "To host the shared Agent Brain service (trace/index infrastructure a team-scale Brain runs on) as it moves from ideation to a real pilot — the Founders Hub tiers (up to $150K) map directly onto that build-out, plus Azure OpenAI Service credits for provider-side experimentation alongside our own BYOK model."),
    ("Go-to-market + technical pairing", "Founders Hub's Azure architecture pairing and Microsoft's enterprise sales network are exactly the muscle a VP-Eng-facing \"Team Agent Brain\" motion needs — plus GitHub Enterprise for our own CI as we scale past a single-founder repo."),
]
